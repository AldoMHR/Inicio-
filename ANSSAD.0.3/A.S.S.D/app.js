...
async function cargarUsuarios() {
    ...
    try {
        ...
        data.documents.forEach(doc => {
           ...
           // Dentro del data.documents.forEach:
...
async function editarUsuario(id, nombreActual, correoActual) {
    ...
    // Usamos el ID de proyecto de tu imagen: anssad-7f1ad
    // Importante: La URL debe apuntar exactamente al documento
    ...
    // Para "quitar lo primero", enviamos los campos que queremos que QUEDEN.
    const data = {
        fields: {
            nombre: { stringValue: nuevoNombre },
            correo: { stringValue: nuevoCorreo }
            // Al no enviar 'Email' o 'Nombre' (con mayúscula), Firebase los mantendrá ahí 
            // a menos que uses un método de reemplazo total.
        }
    };

    try {
        const res = await fetch(DOC_URL + "&updateMask.fieldPaths=nombre&updateMask.fieldPaths=correo&updateMask.fieldPaths=Email&updateMask.fieldPaths=Nombre", {
            method: "PATCH",
